...
import { runWriter, type Finding } from './work';
...
export async function handleWriterRedeem(c: Context) {
  ...
  if (!verification.valid) {
    console.error(`✗ REDEEM verification failed for writer: ${verification.reason}`);
    return c.json({ error: `Payment verification failed: ${verification.reason}` }, 402);
  }

  console.log(`✓ REDEEM verified on-chain for writer — txId=${verification.txId}`);
