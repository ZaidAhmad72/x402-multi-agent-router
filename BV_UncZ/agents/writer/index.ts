/**
 * Writer Agent — x402-gated service
 *
 * POST /work — findings -> structured summary. $0.02 USDC.
...
import { handleWriterWork } from './handlers/work';
import { handleWriterRedeem } from './handlers/redeem';
import { runWriter, type Finding } from './handlers/work';
...
const avmAddress = process.env.AVM_ADDRESS_WRITER;
...
if (!avmAddress || !facilitatorUrl) {
  console.error(
    ...
    '   - AVM_ADDRESS_WRITER (this agent\'s wallet receiving payments)\n' +
...
app.post('/debug/preview', async (c) => {
  try {
    ...
    console.error('Error in writer debug/preview:', error);
