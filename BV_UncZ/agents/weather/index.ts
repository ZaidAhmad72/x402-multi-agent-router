/**
 * Weather Agent — x402-gated service
 *
 * POST /work — task -> live current weather. $0.02 USDC. Deterministic, NO
 * LLM. Structure mirrors the other agents exactly; only the route table,
 * port, and wallet differ.
 */
...
import { handleWeatherWork } from './handlers/work';
import { handleWeatherRedeem } from './handlers/redeem';
import { runWeather } from './handlers/work';
...
const avmAddress = process.env.AVM_ADDRESS_WEATHER;
...
if (!avmAddress || !facilitatorUrl) {
  console.error(
    ...
    '   - AVM_ADDRESS_WEATHER (this agent\'s wallet receiving payments)\n' +
...
app.post('/debug/preview', async (c) => {
  try {
    ...
    console.error('Error in weather debug/preview:', error);
