 ...
 */
export async function handleWeatherRedeem(c: Context) {
  const avmAddress = process.env.AVM_ADDRESS_WEATHER as string;
  ...
  if (!verification.valid) {
    console.error(`✗ REDEEM verification failed for weather: ${verification.reason}`);
    return c.json({ error: `Payment verification failed: ${verification.reason}` }, 402);
  }

  console.log(`✓ REDEEM verified on-chain for weather — txId=${verification.txId}`);

  const body = await c.req.json().catch(() => ({}));
  const task: string = typeof body?.task === 'string' ? body.task : '';

  return c.json({
    ...(await runWeather(task)),
