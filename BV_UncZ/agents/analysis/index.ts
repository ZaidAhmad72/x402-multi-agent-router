/**
 * Analysis Agent — x402-gated service
 *
 * POST /work — task (+ optional weather/currency context) -> short
 * plain-language narrative. $0.02 USDC. Structure mirrors the other agents
 * exactly; only the route table, port, and wallet differ.
 */
...
import { handleAnalysisWork } from './handlers/work';
import { handleAnalysisRedeem } from './handlers/redeem';
import { runAnalysis, type WeatherContext, type CurrencyContext } from './handlers/work';
...
const AGENT_NAME = 'analysis';
const avmAddress = process.env.AVM_ADDRESS_ANALYSIS;
...
if (!avmAddress || !facilitatorUrl) {
  console.error(
    ...
    '   - AVM_ADDRESS_ANALYSIS (this agent\'s wallet receiving payments)\n' +
...
console.log('\n' + '═'.repeat(60));
console.log(`x402 AGENT — ${AGENT_NAME.toUpperCase()}`);
...
app.post('/debug/preview', async (c) => {
  try {
    ...
    console.error('Error in analysis debug/preview:', error);
