...
import { runAnalysis, type WeatherContext, type CurrencyContext } from './work';
 ...
 */
export async function handleAnalysisRedeem(c: Context) {
  const avmAddress = process.env.AVM_ADDRESS_ANALYSIS as string;
  ...
  if (!verification.valid) {
    console.error(`✗ REDEEM verification failed for analysis: ${verification.reason}`);
    return c.json({ error: `Payment verification failed: ${verification.reason}` }, 402);
  }

  console.log(`✓ REDEEM verified on-chain for analysis — txId=${verification.txId}`);
  ...
  return c.json({
    ...(await runAnalysis(task, weather, currency)),
